...
export default function PlayerViews({gameInfo, gameMode, userId}){
    ...
    const selectGameView = ()=>{
        ...
        if(gameMode==="select") return <SelectPlayerDeck gameInfo={gameInfo} userId={userId}/>
        if(gameMode==="vote") return <VotePlayerDeck gameInfo={gameInfo} userId={userId}/>
        if(gameMode==="showPoints") return <PointsDisplay gameInfo={gameInfo}/>
    }
    return(
        <div>
            {selectGameView()}
