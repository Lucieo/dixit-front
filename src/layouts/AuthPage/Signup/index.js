...
export default function SignUp(props) {
  ...
  return (
    <div className="container">
      ...
      <p className="red-text" style={{ minHeight: 25 }}>
        {signupErrors}
      </p>
      <form
        onSubmit={(e) => {
      ...
      >
        <input
          ...
          value={name}
          onChange={(e) => {
            checkForm(e.target.value, "name", setName);
          }}
        />
        <input
          placeholder="email"
          ...
          value={email}
          onChange={(e) => {
            checkForm(e.target.value, "email", setEmail);
          }}
        />
        <input
          type="password"
          ...
          value={password}
          onChange={(e) => {
            checkForm(e.target.value, "password", setPassword);
          }}
        />

        <button
          className={`btn ${!isFormValid && "disabled"} signup__validate`}
        >
          Valider
          <i className="material-icons left">check</i>
        </button>
        <p className="signup__switch" onClick={() => props.switchMode()}>
